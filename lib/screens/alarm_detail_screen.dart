    ...
    return Scaffold(
      ...
      body: displayAlarms.isEmpty
          ...
          : ListView.builder(
              ...
              itemBuilder: (context, index) => _buildAlarmCard(context, ref, displayAlarms[index]),
            ),
    );
  }

  Widget _buildAlarmCard(BuildContext context, WidgetRef ref, AlarmEvent alarm) {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Icon(_getSeverityIcon(alarm.severity), color: severityColor, size: 20),
                ...
                const Spacer(),
                if (alarm.status != AlarmStatus.cleared && alarm.status != AlarmStatus.acknowledged)
                  FilledButton.icon(
                    onPressed: () => ref.read(alarmEventsProvider.notifier).acknowledgeAlarm(alarm.id),
                  ...
                  Chip(
                    label: const Text('ACKNOWLEDGED', style: TextStyle(fontSize: 10)),
                    backgroundColor: Colors.blue.withOpacity(0.1),
                    labelStyle: const TextStyle(color: Colors.blue),
                  )
                else
                  Chip(
                    label: const Text('CLEARED', style: TextStyle(fontSize: 10)),
            ...
            Text(alarm.sensorName, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
            ...
            _buildInfoRow('Trigger Value', '${alarm.triggerValue.toStringAsFixed(1)}'),
          ],
        ),
      ),
    );
